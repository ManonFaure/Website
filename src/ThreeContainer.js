...
/**
 * Classe permettant de créer l'arrière plan du site
 * Elle crée une 3D avec la librairie Three.js 
 */
...
class Scene extends Component {
	...
	componentDidMount() {
		...
		const camera = new THREE.PerspectiveCamera(75, width / height, 0.1, 1000) // Initialisation de la caméra
		const renderer = new THREE.WebGLRenderer({ antialias: true }) // Initialisation du rendu 

		// On crée une lumière blanche pour éclairer la scène 
		var light = new THREE.DirectionalLight( 0xffffff );
		light.position.set( 0, 1, 1 ).normalize();
		scene.add(light); // Ajout à la scène
		...
		// On positionne la caméra dans la scène 
		camera.position.z = 2
		camera.position.z = 35

		// Ajout d'un listener permettant d'adapter automatiquement la taille de la scène si la fenêtre est redimensionnée
		...
		// Ajout d'un listener permettant à l'utilisateur de se "promener" autour du système solaire
	...
	/*======================== Planète ==========================*/
	/**
	 * Prototype d'une planète 
	 * @param {img} texture : image de la texture de la planète, ex: planisphère
	 * @param {img} textureBump : image du relief de la planète, s'il n'y en pas mettre la même image que pour la texture
	 * @param {int} radius : taille de la planète
	 * @param {float} posX : position en x dans la scène
	 * @param {float} posY : position en y dans la scène
	 * @param {float} posZ : position en z dans la scène
	 */
	...
	/**
	 * Les cinq fonctions suivantes n'ont pas été créee avec le prototype ci-dessus car elles n'ont pas 
	...
	/**
	 * Les deux fonction ci-dessous gèrent le mouvement des planètes au scroll
	 */
